    ...
    private void ManageInteraction()
        ...
        if(Input.GetAxis("Jump")!=0)
        {
            if(!firePressed)
            {
                ...
                RaycastHit2D[] interationHit = new RaycastHit2D[16];
                List<RaycastHit2D> hitBufferInteraction = new List<RaycastHit2D>(16);
                int count = Physics2D.Raycast(gameObject.transform.position, Vector2.up, contactFilter2DInteraction, interationHit);
                ...
                for (int i = 0; i < count; i++)
                {
                    hitBufferInteraction.Add(interationHit[i]);
                    if(hitBufferInteraction.Count>0)
                    {
                        hitBufferInteraction[0].transform.gameObject.GetComponent<Intro>.Interact();
                   }
                }
            }
            else
